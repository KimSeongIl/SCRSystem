...
<c:set var="list" value="${requestScope.studentUpdate }" />
...
<div id="article">
	...
	<div id="myPagePW">
			<h2>정보수정</h2>
		<form method="post" id="updateForm" action="studentUpdatePro.do">
		<div id="updateTable">
			<table>
				<tr>
					<td>이름</td>
					<td><input type="text" name="uname" value="${ list.name} " class="form-control"></td>				
				</tr>	
				<tr>
					<td>학번</td>
					<td><input type="number" name="uid" value="${ list.studentId}" class="form-control" readonly></td>								
				</tr>		
				<tr>
					<td>비밀번호</td>
				...
				<tr>
					...
					<c:forEach var="i" items="${ department }">
						<c:if test="${i.departmentId eq list.minorId}">
						<option value="${i.departmentId }" selected>${ i.departmentName } </option>
					</c:if>
					<c:if test="${i.departmentId ne list.minorId }">
						<option value="${i.departmentId }">${ i.departmentName } </option>
					</c:if>
					</c:forEach>
				...
				<tr>
					...
					<option value="없음">해당없음</option>
					<c:forEach var="i" items="${ department }">
						<c:if test="${i.departmentId eq list.doubleMajorId}">
						<option value="${i.departmentId }" selected>${ i.departmentName } </option>
					</c:if>
					<c:if test="${i.departmentId ne list.doubleMajorId }">
						<option value="${i.departmentId }">${ i.departmentName } </option>
					</c:if>
					</c:forEach>
			</select></td>
